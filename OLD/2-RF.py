...
# ---- Normalização por prazo (selecione 1 modo) ----
# "anual": usa Ret_Liq_Anual (12m gerencial)
# "12m"  : idêntico a "anual" (mantido por clareza)
# "eaa"  : Equivalent Annual Annuity sobre proxy lifetime
MODO_PRAZO = "eaa"     # "anual" | "12m" | "eaa"
K_EAA = 0.12           # taxa a.a. p/ EAA (ajuste conforme custo de capital alvo)
M_PISO_ANOS = 0.25     # piso p/ prazo médio (evita explosão ao dividir por ~0)
...
# ---- Defaults de delinquency (minúsculos) ----
...
# ---- Helpers de leitura normalizando para minúsculo ----
...
def load_roll_rates_strict(xls, sheet_name, default_df, buckets):
    ...
    # detectar coluna de bucket p/ virar índice
    cand_idx = None
    for c in df.columns:
...
# permite override via 'params'
if params is not None and {"Parametro","Valor"}.issubset(params.columns):
    mparams = params.set_index("Parametro")["Valor"].to_dict()
    try:
        funding_aa  = float(mparams.get("Funding_aa", FUNDING_AA_DEFAULT))
    except Exception:
        pass
    try:
        H_FWD_MESES = int(mparams.get("H_fwd_meses", H_FWD_MESES_DEFAULT))
    except Exception:
        pass
    # opcionais: modo_prazo, K_EAA
    if "Modo_prazo" in mparams:
        MODO_PRAZO = str(mparams["Modo_prazo"]).strip().lower()
    if "K_EAA" in mparams:
        try:
            K_EAA = float(mparams["K_EAA"])
        except Exception:
            pass

# ---------- PATCH ROBUSTO Spread_aa / colunas ----------
...
# "Na Safra" => Meses_Pagos = 0
contratos["Meses_Pagos"] = contratos.apply(
    lambda r: 0 if r["Estado"]=="Na Safra" else min(max(int(r["Meses_Pagos"]), 0), int(r["Prazo_Meses"])-1),
    axis=1
)
...
# Indicadores comportamentais (fallback heurístico)
...
for ghr, sub in contratos.groupby("GHR"):
    ...
    # PD ajustado por comportamento
    pd_adj = pd_dinamico(pd_base, overs, isdel)

    # EL gerencial do mês (provisão incorrida + ECL forward + carry) e LGD/rho efetivos ponderados
    el_mes_total = 0.0
    ...
    # Retorno “anual gerencial” (12m) por coerência com carry e ECL (horizonte H_FWD_MESES)
...
# ---------- 3.1) Seleção da métrica de retorno (conforme MODO_PRAZO) ----------
# Proxy "lifetime": anual * anos (sem desconto). O EAA corrige pelo tempo via K_EAA.
...
# ---------- 4) Fronteira e Eficiência (com Ret_Metrica) ----------
...
fr_ec, fr_ret, ecs_cloud, rets_cloud = construir_fronteira(
    ...
    RetLiq_1y=df_ghr["Ret_Metrica"].values,   # métrica selecionada
    rho_mat=rho_mat,
    n_points=30000,
    seed=SEED,
)
...
# ---------- 5) Gráficos (Eficientes x Ineficientes, com labels) ----------
...
# eficientes (verde)
ax.scatter(df_ghr.loc[mask_ok, "EC_Ajustado"]/1e6,
           df_ghr.loc[mask_ok, "Ret_Metrica"]/1e6,
           s=80, marker='o', label="GHRs Eficientes (≥1)", color="#2ca02c")
_label_points(ax,
              df_ghr.loc[mask_ok,"EC_Ajustado"]/1e6,
              df_ghr.loc[mask_ok,"Ret_Metrica"]/1e6,
              df_ghr.loc[mask_ok].index, color="#2ca02c")

# ineficientes (azul)
ax.scatter(df_ghr.loc[mask_bad, "EC_Ajustado"]/1e6,
           df_ghr.loc[mask_bad, "Ret_Metrica"]/1e6,
           s=90, marker='x', label="GHRs Ineficientes (<1)", color="#1f77b4")
_label_points(ax,
              df_ghr.loc[mask_bad,"EC_Ajustado"]/1e6,
              df_ghr.loc[mask_bad,"Ret_Metrica"]/1e6,
              df_ghr.loc[mask_bad].index, color="#1f77b4")
...
_label_points(ax,
              df_ghr["EC_Ajustado"]/1e6,
              (df_ghr["ROE_Ajustado"]*100),
              df_ghr.index, color="#333333")
...
# ================================
# 5-bis) Helpers p/ análises extras (sem safra / só safra)
# ================================
def agrega_e_fronteira_subset(subset, label_subset, seed=SEED):
    ...
    for ghr, sub in subset.groupby("GHR"):
        ...
        ret_liq_anual = spread * ead - el_mes_total   # anual gerencial (12m)
        agg_loc.append({
            "GHR": ghr, "EAD": ead, "Volume_Contratos": vol,
            "Overs": overs, "Is_Delayed": isdel, "M_Rem_anos": m_rem_anos,
            "PD_Base": pd_base, "PD_Ajustado": pd_adj,
    ...
    # ---- Fronteira desse subset, usando Ret_Metrica ----
    ...
    fr_ec_loc, fr_ret_loc, ecs_cloud_loc, rets_cloud_loc = construir_fronteira(
        EAD=df_loc["EAD"].values,
        LGD=df_loc["LGD_Efetivo"].values,
...
def plot_subset(res, fname_suffix, y_label_ret):
    df_loc = res["df_ghr"]
    fr_ec_loc, fr_ret_loc = res["fr_ec"], res["fr_ret"]
    ecs_cloud_loc, rets_cloud_loc = res["ecs_cloud"], res["rets_cloud"]

    if df_loc.empty:
        print(f"[{res['label']}] Subconjunto vazio; gráficos não gerados.")
        return

    mask_ok  = df_loc["Eficiência"] >= 1.0
    mask_bad = ~mask_ok
    ...
    ax.set_xlabel("Capital Econômico Ajustado (R$ milhões)")
    ax.set_ylabel(y_label_ret)
    ax.set_title(f"Risk Frontier – {res['label']}")
    ax.legend(); ax.grid(True); plt.tight_layout()
    plt.savefig(OUTPUT_DIR / f"fronteira_{fname_suffix}.png", dpi=200)
    plt.show()
    ...
    ax.set_xlabel("Capital Econômico Ajustado (R$ milhões)")
    ax.set_ylabel("ROE Ajustado ao Risco (% a.a.)")
    ax.set_title(f"ROE Ajustado vs ROE Máximo – {res['label']}")
    ax.legend(); ax.grid(True); plt.tight_layout()
    plt.savefig(OUTPUT_DIR / f"roe_maximo_{fname_suffix}.png", dpi=200)
    plt.show()

# ================================
# 5-ter) Gerar visões extras
# ================================
...
# ================================
# 6) Salvar Excel (atualizado com abas extras)
# ================================
...
with pd.ExcelWriter(OUT_XLSX, engine="xlsxwriter") as writer:
    ...
    # tabelas auxiliares efetivamente usadas
